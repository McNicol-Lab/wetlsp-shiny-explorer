...
server <- function(input, output, session) {
  ...
  observeEvent(input$pixel_map_draw_new_feature, {
    ...
    # Works for rectangle and polygon draw tools. Coordinates are lon/lat.
    coords <- feature$geometry$coordinates[[1]]
    lng <- vapply(coords, function(z) as.numeric(z[[1]]), numeric(1))
    lat <- vapply(coords, function(z) as.numeric(z[[2]]), numeric(1))

    inside <- geom_all |>
      dplyr::filter(
        .data$lon >= min(lng, na.rm = TRUE),
        .data$lon <= max(lng, na.rm = TRUE),
        .data$lat >= min(lat, na.rm = TRUE),
        .data$lat <= max(lat, na.rm = TRUE)
      ) |>
      dplyr::pull(.data$pixel_id) |>
      as.character()
  ...
  output$pixel_map <- renderLeaflet({
    ...
    dat <- tryCatch(evi_data(), error = function(e) NULL)
    sampled_geom <- if (!is.null(dat) && "geometry" %in% names(dat)) dat$geometry else tibble::tibble()

    selected_ids <- as.character(selected_pixel_ids())
    selected_geom <- all_geom |>
      dplyr::filter(as.character(.data$pixel_id) %in% selected_ids)

    # Mark current query/sample pixels so they can be highlighted over the full pixel cloud.
    all_geom <- all_geom |>
      dplyr::mutate(
        is_current = .data$pixel_id %in% sampled_geom$pixel_id,
        is_selected = as.character(.data$pixel_id) %in% selected_ids
      )

    map <- leaflet(options = leafletOptions(preferCanvas = TRUE)) |>
      add_reliable_base_tiles() |>
      addCircleMarkers(
        data = all_geom |> dplyr::filter(!.data$is_current, !.data$is_selected),
        lng = ~lon, lat = ~lat,
        radius = 2,
        stroke = FALSE,
        fillColor = "#bdbdbd",
        ...
        group = "All other pixels"
      )

    if (nrow(sampled_geom) > 0) {
      map <- map |>
        addCircleMarkers(
          data = sampled_geom,
          lng = ~lon, lat = ~lat,
    ...
    map |>
      ...
      addLayersControl(
        baseGroups = c("OpenStreetMap", "CartoDB Positron", "Esri World Imagery"),
        overlayGroups = c("All other pixels", "Current EVI pixels", "Selected pixels", "Selection boxes"),
        options = layersControlOptions(collapsed = TRUE)
      ) |>
      fitBounds(
