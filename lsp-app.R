...
ui <- page_sidebar(
  title = "LSP PlanetScope EVI Explorer",
  ...
  sidebar = sidebar(
    ...
    numericInput(
      "selection_pixel_limit",
      "Maximum selected pixels to keep",
      value = 500,
      min = 25,
      max = 5000,
      step = 25
    ...
    helpText(
      "Pixel selection: on the Pixel map, use the rectangle/polygon tool to select a group, or click individual pixels to toggle them."
    ),
    dateRangeInput(
      "date_range",
      "Date range",
...
read_evi_subset_for_pixels <- function(
  ...
  max_pixels <- suppressWarnings(as.integer(max_pixels))
  if (is.na(max_pixels) || max_pixels < 1) max_pixels <- 250L

  # Use selected IDs only after strict cleaning/downsampling.
  if (!is.null(selected_pixel_ids) && length(selected_pixel_ids) > 0) {
    pixel_ids <- unique(as.character(selected_pixel_ids))
    pixel_ids <- pixel_ids[!is.na(pixel_ids) & nzchar(pixel_ids)]

    # Preserve numeric IDs if the source schema is integer.
    pixel_ids_num <- suppressWarnings(as.integer(pixel_ids))
    pixel_ids_num <- pixel_ids_num[!is.na(pixel_ids_num)]

    if (length(pixel_ids_num) == 0) {
      pixel_ids <- character()
    } else {
      pixel_ids <- head(pixel_ids_num, max_pixels)
    }
  } else {
    pixel_ids <- ts_ds |>
      dplyr::filter(.data$year == !!as.integer(year), .data$series %in% !!series) |>
      dplyr::select(.data$pixel_id) |>
      dplyr::distinct() |>
      head(max_pixels) |>
      dplyr::collect() |>
...
phenometric_scale_type <- function(layer_name) {
  # LSP / Moon et al. 2022 product names:
  # Timing metrics: OGI, 50PCGI, OGMx, Peak, OGD, 50PCGD, OGMn and *_2.
  # Vegetation indices: EVImax, EVIamp, EVIarea and *_2.
  # QA/count products: QA, QA_2, NumCycles, numObs.
  nm <- layer_name %||% ""
  nm_clean <- gsub("^.*\\|\\s*", "", nm)
  nm_clean <- trimws(nm_clean)
  nm_lower <- tolower(nm_clean)

  timing_layers <- tolower(c(
    "OGI", "50PCGI", "OGMx", "Peak", "OGD", "50PCGD", "OGMn",
    "OGI_2", "50PCGI_2", "OGMx_2", "Peak_2", "OGD_2", "50PCGD_2", "OGMn_2"
  ))

  greenness_layers <- tolower(c(
    "EVImax", "EVIamp", "EVIarea",
    "EVImax_2", "EVIamp_2", "EVIarea_2"
  ))

  qa_layers <- tolower(c("QA", "QA_2"))
  count_layers <- tolower(c("NumCycles", "numObs"))

  if (nm_lower %in% timing_layers) return("timing")
  if (nm_lower %in% greenness_layers) return("greenness")
  if (nm_lower %in% qa_layers) return("qa")
  if (nm_lower %in% count_layers) return("count")

  # Fallbacks for NetCDF drivers that alter layer names.
  if (grepl("^(ogi|50pcgi|ogmx|peak|ogd|50pcgd|ogmn)(_2)?$", nm_lower)) return("timing")
  if (grepl("^evi(max|amp|area)(_2)?$", nm_lower)) return("greenness")
  if (grepl("^qa(_2)?$", nm_lower)) return("qa")
  if (grepl("^(numcycles|numobs)$", nm_lower)) return("count")

  "generic"
}


lsp_layer_dictionary <- function() {
  tibble::tribble(
    ~layer, ~description, ~units, ~scale_factor, ~valid_range,
    "NumCycles", "Number of phenological cycles detected in target year", "number of cycles", 1, "0-6",
    "OGI", "Onset Greenness Increase; date of 15% greenness increase", "day of year", 1, "-181-548",
    "50PCGI", "50 Percent Greenness Increase; date of 50% greenness increase", "day of year", 1, "-181-548",
    "OGMx", "Onset Greenness Maximum; date of 90% greenness increase", "day of year", 1, "-181-548",
    "Peak", "Date of cycle peak", "day of year", 1, "1-366",
    "OGD", "Onset Greenness Decrease; date of 10% greenness decrease", "day of year", 1, "-181-548",
    "50PCGD", "50 Percent Greenness Decrease; date of 50% greenness decrease", "day of year", 1, "-181-548",
    "OGMn", "Onset Greenness Minimum; date of 85% greenness decrease", "day of year", 1, "-181-548",
    "EVImax", "Maximum EVI2 during vegetation cycle", "unitless", 0.0001, "0-10000",
    "EVIamp", "EVI2 amplitude during vegetation cycle", "unitless", 0.0001, "0-10000",
    "EVIarea", "Integrated EVI2 during vegetation cycle", "unitless", 0.0001, "0-32766",
    "OGI_2", "Second cycle onset greenness increase", "day of year", 1, "-181-548",
    "50PCGI_2", "Second cycle 50 percent greenness increase", "day of year", 1, "-181-548",
    "OGMx_2", "Second cycle onset greenness maximum", "day of year", 1, "-181-548",
    "Peak_2", "Second cycle date of cycle peak", "day of year", 1, "1-366",
    "OGD_2", "Second cycle onset greenness decrease", "day of year", 1, "-181-548",
    "50PCGD_2", "Second cycle 50 percent greenness decrease", "day of year", 1, "-181-548",
    "OGMn_2", "Second cycle onset greenness minimum", "day of year", 1, "-181-548",
    "EVImax_2", "Second cycle maximum EVI2", "unitless", 0.0001, "0-10000",
    "EVIamp_2", "Second cycle EVI2 amplitude", "unitless", 0.0001, "0-10000",
    "EVIarea_2", "Second cycle integrated EVI2", "unitless", 0.0001, "0-32766",
    "QA", "Quality assurance for first vegetation cycle", "class", 1, "1-4",
    "QA_2", "Quality assurance for second vegetation cycle", "class", 1, "1-4",
    "numObs", "Number of days with clear observations in calendar year", "days", 1, "0-366"
  )
}

lsp_layer_label <- function(layer_name) {
  dict <- lsp_layer_dictionary()
  row <- dict |> dplyr::filter(.data$layer == layer_name) |> dplyr::slice(1)
  if (nrow(row) == 0) return(layer_name)
  paste0(layer_name, " — ", row$description[[1]])
}
...
plot_netcdf_layer <- function(df, title = NULL, layer_name = NULL) {
  ...
  if (identical(scale_type, "timing")) {
    # Moon et al.-style high-contrast palette for day-of-year timing phenometrics.
  ...
  } else if (identical(scale_type, "qa")) {
    # QA classes are 1--4 in the product table.
    p <- p +
      ggplot2::scale_fill_stepsn(
        colours = c("#f7f7f7", "#cccccc", "#969696", "#525252"),
        breaks = c(1, 2, 3, 4),
        na.value = "transparent",
        guide = ggplot2::guide_coloursteps(title.position = "top")
      )
  } else if (identical(scale_type, "count")) {
    # Count products: NumCycles = 0--6; numObs = 0--366.
    p <- p +
...
server <- function(input, output, session) {
  ...
  observeEvent(input$pixel_map_marker_click, {
    px <- input$pixel_map_marker_click$id
    if (!is.null(px) && nzchar(px)) {
      ...
      if (px %in% current) {
        ...
        capped <- cap_selected_pixels(unique(c(current, px)), (input$selection_pixel_limit %||% 500))
  ...
  observeEvent(input$pixel_map_draw_new_feature, {
    ...
    limit <- (input$selection_pixel_limit %||% 500)
    ...
    inside <- tryCatch({
      ...
      # Cheap bounding-box prefilter first. This prevents sf from testing all pixels
      # for large sites and avoids freezing the app for broad polygons.
      candidates <- geom_all |>
        dplyr::filter(
          !is.na(.data$lon), !is.na(.data$lat),
          .data$lon >= min(xy[, 1], na.rm = TRUE),
          .data$lon <= max(xy[, 1], na.rm = TRUE),
          .data$lat >= min(xy[, 2], na.rm = TRUE),
          .data$lat <= max(xy[, 2], na.rm = TRUE)
        )

      if (nrow(candidates) == 0) return(character())

      # If the bbox candidate set is very large, down-cap before more expensive
      # geometry operations. This keeps the Shiny session responsive.
      if (nrow(candidates) > as.integer(limit) * 10L) {
        showNotification(
          paste0("Large selection area: ", nrow(candidates), " bbox candidate pixels. Using capped subset."),
          type = "warning",
          duration = 6
        )
        candidates <- candidates |> dplyr::slice_head(n = as.integer(limit) * 10L)
      }
      ...
      if (requireNamespace("sf", quietly = TRUE)) {
        ...
        # Fallback: bbox only if sf is unavailable.
        candidates |>
          dplyr::pull(.data$pixel_id) |>
          as.character()
      }
      ...
      showNotification(
        paste0("Pixel draw selection failed: ", conditionMessage(e)),
        type = "error",
        duration = 8
      )
      character()
    ...
    capped <- cap_selected_pixels(new_selection, limit)

    if (isTRUE(capped$was_limited)) {
      showNotification(
        paste0(
          "Selection returned ", capped$n_original, " pixels; keeping first ",
          capped$n_kept, " to protect app responsiveness."
        ),
        type = "warning",
        duration = 8
      )
  ...
  output$site_info <- renderDT({
    selected_site() |>
      select(any_of(c(
        ...
        "lsp_2021", "lsp_2022", "lsp_2023", "lsp_2024"
      ))) |>
  ...
  evi_data <- eventReactive(input$load_data, {
    ...
    selected_ids <- NULL
    if (isTRUE(input$use_pixel_selection) && length(selected_pixel_ids()) > 0) {
      selected_ids <- unique(as.character(selected_pixel_ids()))
      selected_ids <- selected_ids[!is.na(selected_ids) & nzchar(selected_ids)]

      # Critical stability guard:
      # A polygon can select thousands of pixels. Even if the selection overlay is fine,
      # loading pixel x daily EVI for all selected pixels can terminate the R session.
      max_n <- suppressWarnings(as.integer(input$max_pixels))
      if (is.na(max_n) || max_n < 1) max_n <- 250L

      if (length(selected_ids) > max_n) {
        showNotification(
          paste0("Using ", max_n, " selected pixels out of ", length(selected_ids),
                 " selected. Increase the random-pixel slider if you want more."),
          type = "warning",
          duration = 7
        )
        selected_ids <- selected_ids[seq_len(max_n)]
      }
    }
    ...
    tryCatch({
      ...
      showNotification(
        paste0("Could not load selected pixel time series: ", conditionMessage(e)),
        type = "error",
        duration = 10
      )
      list(
        ...
        geometry = tibble::tibble(),
        paths = list(
          timeseries = resolve_timeseries_path(cfg, site),
          geometry = resolve_geom_path(cfg, site)
        ),
  ...
  output$evi_plot <- renderPlot({
    ...
    validate(need(!is.null(ts) && nrow(ts) > 0, "No rows returned for this filter, or selected pixel loading failed."))
  ...
  output$pheno_layer_ui <- renderUI({
    ...
    layer_choices <- stats::setNames(layers, vapply(layers, lsp_layer_label, character(1)))
