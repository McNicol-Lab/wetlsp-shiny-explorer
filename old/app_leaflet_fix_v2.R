...
server <- function(input, output, session) {
  ...
  output$pixel_map <- renderLeaflet({
    ...
    site <- selected_site()
    all_geom <- read_all_pixel_geometry(cfg, site)

    validate(need(nrow(sampled_geom) > 0, "No sampled geometry rows returned."))
    validate(need(nrow(all_geom) > 0, "No full-site geometry rows returned."))

    # Mark sampled pixels so they can be highlighted over the full pixel cloud.
    all_geom <- all_geom |>
      mutate(is_sampled = .data$pixel_id %in% sampled_geom$pixel_id)

    leaflet(options = leafletOptions(preferCanvas = TRUE)) |>
      add_reliable_base_tiles() |>
      addCircleMarkers(
        data = all_geom |> filter(!.data$is_sampled),
        lng = ~lon, lat = ~lat,
        radius = 2,
        stroke = FALSE,
        fillColor = "#9e9e9e",
      ...
      addCircleMarkers(
        ...
        fillColor = "#1f78b4",
        fillOpacity = 0.85,
        label = ~paste0("sampled pixel_id: ", pixel_id),
        group = "Sampled pixels"
      ) |>
      addLayersControl(
        baseGroups = c("OpenStreetMap", "CartoDB Positron", "Esri World Imagery"),
        overlayGroups = c("All other pixels", "Sampled pixels"),
        options = layersControlOptions(collapsed = TRUE)
      ) |>
      fitBounds(
